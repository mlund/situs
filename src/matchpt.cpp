...
;
  ...
  for(int i=4; i<argc; i++) {
	...
	if (iranking < 0) { iranking = 0; MPTO << "Warning: -ranking must be in (0,...,5), selecting 0" << endl; }
	if (iranking > 5) { iranking = 5; MPTO << "Warning: -ranking must be in (0,...,5), selecting 5" << endl; }
	if (oVOL.getSizeX() == 0 && (iranking == 1 || iranking > 2)) { iranking = 0;  MPTO << "Warning: No map loaded, unable to rank by CC, using default RMSD criterion" << endl; }
	if (oPDB.size() == 0 && (iranking == 1 || iranking > 2)) { iranking = 0;  MPTO << "Warning: No structure loaded, unable to rank by CC, using default RMSD criterion" << endl; }
	i++;
	
	if (bCV == false) {
	  switch (iranking) {
	  case 0: MPTO << "Ranking option set to " << iranking << " (min RMSD / RMSD)" << endl; break;
	  case 1: MPTO << "Ranking option set to " << iranking << " (min RMSD / cross correlation)" << endl; break;
	  case 2: MPTO << "Ranking option set to " << iranking << " (min combined variabilities / RMSD)" << endl; break;
	  case 3: MPTO << "Ranking option set to " << iranking << " (min combined variabilities / cross correlation)" << endl; break;
	  case 4: MPTO << "Ranking option set to " << iranking << " (max cross correlation / RMSD)" << endl; break;
	  case 5: MPTO << "Ranking option set to " << iranking << " (max cross correlation / cross correlation)" << endl; break;
	  }
	} else {
	  switch (iranking) {
	  case 0: case 2: case 4: MPTO << "Ranking option set to " << iranking << " (solutions ranked by RMSD)" << endl; break;
	  case 1: case 3: case 5: MPTO << "Ranking option set to " << iranking << " (solutions ranked by cross correlation)" << endl; break;
	  }
	}
      }
    }
...
#pragma omp parallel for schedule(dynamic)
  ...
  if (aMatches.size() > 0) {
    ...
    if (bCV == false) {
      ...
      switch (iranking) {
      case 0: case 1: sprintf(pStr2, "min RMSD"); break;
      case 2: case 3: sprintf(pStr2, "min combined variabilities"); break;
      case 4: case 5: sprintf(pStr2, "max CC"); break;
      }
    ...
    switch (iranking) {
    case 0: case 2: case 4: sprintf(pStr4, " RMSD"); break;
    case 1: case 3: case 5: sprintf(pStr4, " CC"); break;
    }
