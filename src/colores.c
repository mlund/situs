...
* (c) Pablo Chacon, Jochen Heyd, Valerio Mariani, and Willy Wriggers * 
* 2001-2009                                                          *
**********************************************************************
*                                                                    *
...
static fftwnd_plan g_fftw_plan_forward;      /* forward FFT plan setup */
static fftwnd_plan g_fftw_plan_reverse;      /* reverse FFT plan setup */
...
typedef struct { unsigned eu; float score; } SAV;
...
int main(int argc, char **argv) { 
 ...
 /*========================= INITIALIZATION =================================*/
  ...
  do_vect(&g_phi_lo,g_fftw_nvox_r2c);
  ...
  /* precalculate optimal FFTW routines for given grid size */
  g_fftw_plan_forward = rfftw3d_create_plan(g_extz, g_exty, g_extx,
			      FFTW_REAL_TO_COMPLEX, FFTW_MEASURE);
  g_fftw_plan_reverse = rfftw3d_create_plan(g_extz, g_exty, g_extx, 
 ...
 /*============================== TESTING ===================================*/
  ...
  /* [FFT(A)] */ 
  rfftwnd_one_real_to_complex(g_fftw_plan_forward, g_phi_lo, g_fftw_grid_a);
  /* [FFT(B)] */
  rfftwnd_one_real_to_complex(g_fftw_plan_forward, g_phi_hi, g_fftw_grid_b); 
  /* C [IFFT(BxA*)] */ 
  for (q=0;q<g_fftw_nvox_c2r;q++) {
    save_b_re=g_fftw_grid_b[q].re;
    g_fftw_grid_b[q].re =(g_fftw_grid_a[q].re * g_fftw_grid_b[q].re + g_fftw_grid_a[q].im * g_fftw_grid_b[q].im)*g_fftw_scale;
    g_fftw_grid_b[q].im =(g_fftw_grid_a[q].re * g_fftw_grid_b[q].im - g_fftw_grid_a[q].im * save_b_re)*g_fftw_scale;
  }  

  /* compute inverse FFT */
  zero_vect(g_phi_hi, g_fftw_nvox_r2c);
  rfftwnd_one_complex_to_real(g_fftw_plan_reverse, g_fftw_grid_b, g_phi_hi);
  ...
  g_eulers_count=0; 
  switch (g_euler_mode) {
  case 0:eu_proportional (g_eu_range, g_delta_rot, &g_eulers_count, &g_eulers); break;
  case 1:eu_sparsed (g_eu_range, g_delta_rot, &g_eulers_count, &g_eulers); break;
  case 2:eu_spiral  (g_eu_range, g_delta_rot, &g_eulers_count, &g_eulers); break;
  case 3:read_eulers (g_eu_in_file, &g_eulers_count, &g_eulers);  break;
  }
  ...
  /* free memory */
  free(g_fftw_grid_a); free(g_fftw_grid_b);
  rfftwnd_destroy_plan(g_fftw_plan_forward);
  rfftwnd_destroy_plan(g_fftw_plan_reverse);
...
/* carries out a FFT scan of all Euler angles; requires all global variables to be set */   
  ...
  /* [FFT(B)] */
  rfftwnd_one_real_to_complex(g_fftw_plan_forward, g_phi_du, g_fftw_grid_b); 
  ...
  for (m=0;m<g_inside_num_flipped;m++) {
    ...
    if (curr_score>g_hash_sav[q].score) {g_hash_sav[q].score=curr_score; g_hash_sav[q].eu=i;}
    if (curr_score>max_score) {
      max_score= curr_score;
      grid_pos[0]=g_inside_list_flipped[m].ix; grid_pos[1]=g_inside_list_flipped[m].iy; grid_pos[2]=g_inside_list_flipped[m].iz;
    }    
  }
...
static void *search6d_fft_par(void *thread_arg) {
  ...
  fftwnd_plan p_fftw_plan_forward;      /* forward FFT plan setup */
  fftwnd_plan p_fftw_plan_reverse;      /* reverse FFT plan setup */
  ...
  p_fftw_plan_forward = rfftw3d_create_plan(g_extz, g_exty, g_extx,
			      FFTW_REAL_TO_COMPLEX, FFTW_MEASURE);
  p_fftw_plan_reverse = rfftw3d_create_plan(g_extz, g_exty, g_extx, 
			      FFTW_COMPLEX_TO_REAL, FFTW_MEASURE);
  ...
  for(i=idx_start; i<idx_start+idx_count; i++){
    ...
    /* [FFT(B)] */
    rfftwnd_one_real_to_complex(p_fftw_plan_forward, p_phi_du, p_fftw_grid_b); 

    /* C [IFFT(BxA*)] */ 
    for (q=0;q<g_fftw_nvox_c2r;q++) {
      save_b_re=p_fftw_grid_b[q].re;
      p_fftw_grid_b[q].re =(g_fftw_grid_a[q].re * p_fftw_grid_b[q].re 
                            + g_fftw_grid_a[q].im * p_fftw_grid_b[q].im)*g_fftw_scale;
      p_fftw_grid_b[q].im =(g_fftw_grid_a[q].re * p_fftw_grid_b[q].im 
                             - g_fftw_grid_a[q].im * save_b_re)*g_fftw_scale; 
    }  
    ...
    rfftwnd_one_complex_to_real(p_fftw_plan_reverse, p_fftw_grid_b, p_phi_hi);
...
/* print usage info and read options from input arguments */
  ...
  switch (g_corr_mode) {
  case 0: printf("colores> Standard cross correlation\n"); break;
  case 1: printf("colores> Laplacian filtered correlation\n"); break;
  default:
  ...
  switch (g_euler_mode) {
  case 0: printf("colores> Euler angles generation using Proportional method\n"); break;
  case 1: printf("colores> Euler angles generation using Lattman et al. method\n"); break;
  case 2: printf("colores> Euler angles generation using Spiral method\n"); break;
  case 3: {
  ...
  switch (g_peak_opt) {
  case 0: printf("colores> Original peak search by sort and filter\n"); break;
  case 1: printf("colores> New peak search by filter only\n"); break;
  default:
    error_option(80420, g_program);
  }
...
static void peak_extract (SAV **ccr, FIT **found_peak_final) {
  ...
  double smooth_filter[3][3][3] = {{{0.02777777777778,0.02777777777778,0.02777777777778},
...
/* writes list to g_inside_list number to g_inside_num                      */
  ...
  for (iz=0;iz<g_extz;iz++)
	...
	if (mask_inside[q]==1) {
	  ...
	  if ((check > 0)&&(mask_inside2[q] == 1)) {mask_inside2[q]=0; g_inside_num--;}
	}       
      }   
  free(mask_inside);
...
/* writes list to g_inside_list_flipped, number to g_inside_num_flipped     */
  ...
  for (iz=0;iz<g_extz;iz++)
	...
	q=ix+(g_extx)*(iy+g_exty*iz);       /* actual lattice index */
	if (phi[q]>0.000) { /* inside or on contour */    
	  mask_inside[q]=1; g_inside_num_flipped++; 
	} else { /* scan in all directions to check if inside */  
	  check=0;
	  for(i=0;i<ix;i++) 
	    if (phi[i+(g_extx)*(iy+g_exty*iz)]>0.0000) {
  ...
  for (iz=0;iz<g_extz;iz++)
	...
	if (mask_inside[q]==1) {
	  ...
	  if ((check > 0)&&(mask_inside2[q] == 1)) {mask_inside2[q]=0; g_inside_num_flipped--;}
	}       
      }   
  free(mask_inside);
 ...
 // ADDED LINES FINISH
  ...
  free(mask_inside3); // THIS LINE HAS BEEN MODIFIED
}
