...
int main(int argc, char *argv[]) {
  double *phi1, *phi2, *phi3, *pphi1;
	...
	for (i=2;i<(argc-1);++i) {
	  ...
	  if (origx2<origx1) minx = 0 - (int) ceil((origx1-origx2)/width1); else minx = 0;
	  if (origy2<origy1) miny = 0 - (int) ceil((origy1-origy2)/width1); else miny = 0;
	  if (origz2<origz1) minz = 0 - (int) ceil((origz1-origz2)/width1); else minz = 0;  
	  maxx = extx1-1 + (int) ceil(((origx2+(extx2-1)*width2)-(origx1+(extx1-1)*width1))/width1); if (maxx<extx1) maxx = extx1-1;
	  maxy = exty1-1 + (int) ceil(((origy2+(exty2-1)*width2)-(origy1+(exty1-1)*width1))/width1); if (maxy<exty1) maxy = exty1-1;
	  maxz = extz1-1 + (int) ceil(((origz2+(extz2-1)*width2)-(origz1+(extz1-1)*width1))/width1); if (maxz<extz1) maxz = extz1-1;     
	...
	for(count=0;count<nvox1;count++) *(phi1+count) /= (1.0*(argc-2));
