 ...
 */
double vec4::x() const { return (*this)[0]; }
void vec4::x(double value) { (*this)[0] = value; }

double vec4::y() const { return (*this)[1]; }
void vec4::y(double value) { (*this)[1] = value; }

double vec4::z() const { return (*this)[2]; }
void vec4::z(double value) { (*this)[2] = value; }

double vec4::w() const { return (*this)[3]; }
void vec4::w(double value) { (*this)[3] = value; }
...
volume* pointcloud::blur(double fWidth, double fResolution )
{
    ...
    for (i=0; i<iAtomsNum; i++)
    {
        ...
        pMap->setValue(iX0, iY0, iZ0, pMap->getValue(iX0, iY0, iZ0) + fA * fB * fC                 * getMass(i)); fVarp += getMass(i) * fA * fB * fC * ((1.0-fA)*(1.0-fA)+(1.0-fB)*(1.0-fB)+(1.0-fC)*(1.0-fC));
        pMap->setValue(iX0, iY0, iZ1, pMap->getValue(iX0, iY0, iZ1) + fA * fB * (1.0-fC)           * getMass(i)); fVarp += getMass(i) * fA * fB * (1.0-fC) * ((1.0-fA)*(1.0-fA)+(1.0-fB)*(1.0-fB)+fC*fC);
        pMap->setValue(iX0, iY1, iZ0, pMap->getValue(iX0, iY1, iZ0) + fA * (1-fB) * fC             * getMass(i)); fVarp += getMass(i) * fA * (1.0-fB) * fC * ((1.0-fA)*(1.0-fA)+fB*fB+(1.0-fC)*(1.0-fC));
        pMap->setValue(iX1, iY0, iZ0, pMap->getValue(iX1, iY0, iZ0) + (1.0-fA) * fB * fC           * getMass(i)); fVarp += getMass(i) * (1.0-fA) * fB * fC * (fA*fA+(1.0-fB)*(1.0-fB)+(1.0-fC)*(1.0-fC));
        pMap->setValue(iX0, iY1, iZ1, pMap->getValue(iX0, iY1, iZ1) + fA * (1-fB) * (1.0-fC)       * getMass(i)); fVarp += getMass(i) * fA * (1.0-fB) * (1.0-fC) * ((1.0-fA)*(1.0-fA)+fB*fB+fC*fC);
        pMap->setValue(iX1, iY1, iZ0, pMap->getValue(iX1, iY1, iZ0) + (1.0-fA) * (1-fB) * fC       * getMass(i)); fVarp += getMass(i) * (1.0-fA) * (1.0-fB) * fC * (fA*fA+fB*fB+(1.0-fC)*(1.0-fC));
        pMap->setValue(iX1, iY0, iZ1, pMap->getValue(iX1, iY0, iZ1) + (1.0-fA) * fB * (1.0-fC)     * getMass(i)); fVarp += getMass(i) * (1.0-fA) * fB * (1.0-fC) * (fA*fA+(1.0-fB)*(1.0-fB)+fC*fC);
        pMap->setValue(iX1, iY1, iZ1, pMap->getValue(iX1, iY1, iZ1) + (1.0-fA) * (1-fB) * (1.0-fC) * getMass(i)); fVarp += getMass(i) * (1.0-fA) * (1.0-fB) * (1.0-fC) * (fA*fA+fB*fB+fC*fC);
        fSumMass += getMass(i);
    }
    fVarp /= fSumMass;
...
{
	read_vol((char *)pFilename, &m_fWidth, &m_fOrigX, &m_fOrigY, &m_fOrigZ, &m_iSizeX, &m_iSizeY, &m_iSizeZ, &m_pPhi);
	m_bChanged = true;
};
...
double volume::correlation( volume& rVol )
{
    ...
    fCorrelation = (fAddedVoxels*fCoSum - fValueA_Sum*fValueB_Sum)/sqrt((fAddedVoxels*fValueA_Sq_Sum - fValueA_Sum*fValueA_Sum)*(fAddedVoxels*fValueB_Sq_Sum - fValueB_Sum*fValueB_Sum));
                        ...
                        for(iKY = iStart; iKY < iEnd; iKY++)
                            for(iKX = iStart; iKX < iEnd; iKX++)
                            {
                                if ((((int)(iX)+iKX) < (int)(m_iSizeX) && ((int)(iX)+iKX) >= 0) && (((int)(iY)+iKY) < (int)(m_iSizeY) && ((int)(iY)+iKY) >= 0) && (((int)(iZ)+iKZ) < (int)(m_iSizeZ) && ((int)(iZ)+iKZ) >= 0))
                                    pTmp[ (iX+iKX) + ((iY+iKY)*m_iSizeX) + ((iZ+iKZ)*iSizeXY) ] += (rKernel.getValue(((iKX-iStart)*iSizeS)+((iKY-iStart)*iSize)+iKZ-iStart) * fVal);
                            }
                }
