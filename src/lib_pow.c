...
void insert_new_trap_point (double *pf0, double *pf1, double *pf2,
	...
	if(xnew < *px1) {
		if(numcmp(fnew,*pf1) < 0) {
			*px2 = *px1;*pf2 = *pf1;
			*px1 = xnew;*pf1 = fnew;
		} else { *px0 = xnew; *pf0 = fnew; }
	} else if(xnew > *px1) {
		if(numcmp(fnew,*pf1) < 0) {
			*px0 = *px1;*pf0 = *pf1;
			*px1 = xnew;*pf1 = fnew;
		} else { *px2 = xnew; *pf2 = fnew;}
	}
}
...
void bracket_minimum (int *pcode, double *pf0, double *pf1, double *pf2,
	...
	for(;;) {
		...
		if(cmp > 0) xnew = *px2 + widthg;
		else
		if(random_bit()) xnew = *px0 - widthg;
	...
	*pval_min = (*pfunction)(vect);
	...
	for(iteration=0;;++iteration) {
		...
		if(*pval_min == last_val_min) {
			*pcode = 0;
			free(xit); free(ptt);
			free(pt); free(g_buffer_vect);
			for(i=0;i<len;i++) free(xi[i]);
			return;
		}
  ...
  *pval_min = (*pfunction)(vect, scratch);
  ...
  for(iteration=0;;++iteration) {
    ...
    if(*pval_min == last_val_min) {
      *pcode = 0;
      free(xit); free(ptt);
      free(pt); free(scratch->buffer_vect);
      for(i=0;i<len;i++) free(xi[i]);
      return;
    }
    ...
    curr_result = (POWITERRES *) xmalloc(sizeof(POWITERRES));
    curr_result->iter = iteration+1;
    curr_result->corr = -(*pval_min);
    curr_result->res  = (double *) xmalloc(len*sizeof(double));
