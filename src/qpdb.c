...
/* scan PDB and check content */
  ...
  if (nn>NNMAX) {
    nn=NNMAX;
    printf ("...qpdb> Note: maximum number of codebook vectors (defined in situs.h): %d\n",nn); 
  }
  if (nn<NNMIN) {
    nn=NNMIN; 
  ...
  for (i=0;i<num2;++i) {
    ...
    wdiff = 1e20;
    for (j=0;j<nn;++j) {
      currdiff = (wglob[j][0]-pdb2[i].x)*(wglob[j][0]-pdb2[i].x)+(wglob[j][1]-pdb2[i].y)*(wglob[j][1]-pdb2[i].y)+(wglob[j][2]-pdb2[i].z)*(wglob[j][2]-pdb2[i].z);
      if (wdiff > currdiff) {
	wdiff = currdiff;
	k = j;
      }
  ...
  for (i=0;i<num2;++i) {
    effrad[windex[i]] += (wglob[windex[i]][0]-pdb2[i].x)*(wglob[windex[i]][0]-pdb2[i].x)+(wglob[windex[i]][1]-pdb2[i].y)*(wglob[windex[i]][1]-pdb2[i].y)+(wglob[windex[i]][2]-pdb2[i].z)*(wglob[windex[i]][2]-pdb2[i].z);
    mcl[windex[i]]+=1;
  }
  ...
  case 2: case 3: case 4:
    ...
    for (j=0;j<num2;++j) {
      for (i=0;i<nn;++i) dist[i] = (pdb2[j].x-wglob[i][0])*(pdb2[j].x-wglob[i][0]) +
  ...
  case 2:
    ...
    for (j=0;j<num1;++j) {
      ...
      for (i=0;i<nn;++i) dist[i] = (pdb1[j].x-wglob[i][0])*(pdb1[j].x-wglob[i][0]) +
